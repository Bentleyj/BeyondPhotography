...
float yellow = 0.045;
float blue = 0.6; //-0.7
vec3 blueCol = vec3(59./255., 42./255., 39./255.);
vec3 redCol = vec3(254./255., 53./255., 29./255.);
vec3 yellowCol = vec3(250./255., 206./255., 9./255.);
vec3 teal = vec3(6./255., 171./255., 203./255.);
...
void main() {
    ...
    vec3 cG = warholColor(uvAbs, blueCol, blue, width);
    vec3 cY = warholColor(uvAbs, yellowCol, yellow, width);

    float s = sobel(inputTexture, vec2(uvAbs.x, uvAbs.y));

    vec3 col = cR + cG + cY;
    
    if(length(col.rgb) < 1.0)
        col = vec3(231./255., 144./255., 184./255.);
    ...
    vec3 tc = col;//smoothstep(outline, col, vec3(outline.b));
